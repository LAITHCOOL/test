	...
	if (g_ctx.globals.weapon->can_fire(false))
	{
		if (r8cock_time <= server_time)
		{
			if (g_ctx.globals.weapon->m_flNextSecondaryAttack() <= server_time)
				r8cock_time = server_time + TICKS_TO_TIME(12);
			else
				cmd->m_buttons |= IN_ATTACK2;
		}
		else
...
bool aim::automatic_stop(CUserCmd* cmd)
{
	...
	auto max_speed = 0.3f * (g_ctx.globals.scoped ? weapon_info->flMaxPlayerSpeedAlt : weapon_info->flMaxPlayerSpeed);

	if (g_ctx.globals.current_weapon == WEAPON_SSG08)
	{
		max_speed = 0.1f * (g_ctx.globals.scoped ? weapon_info->flMaxPlayerSpeedAlt : weapon_info->flMaxPlayerSpeed);
	}
	else
	{
		max_speed = 0.3f * (g_ctx.globals.scoped ? weapon_info->flMaxPlayerSpeedAlt : weapon_info->flMaxPlayerSpeed);
	}
...
void aim::scan(adjust_data* record, scan_data& data, const Vector& shoot_position, bool optimized)
{
	...
	auto force_safe_points = key_binds::get().get_key_bind_state(3) || g_cfg.player_list.force_safe_points[record->i] || g_cfg.ragebot.weapon[g_ctx.globals.current_weapon].max_misses && g_ctx.globals.missed_shots[record->i] >= g_cfg.ragebot.weapon[g_ctx.globals.current_weapon].max_misses_amount; //-V648
	auto best_damage = 0;
	...
	for (auto& point : points)
	{
		...
		if (fire_data.damage >= current_minimum_damage && fire_data.damage >= best_damage)
		{
			...
			if (record->player->m_iHealth() <= weapon_info->iDamage)
			{
				g_cfg.ragebot.weapon[g_ctx.globals.current_weapon].prefer_body_aim = true;
			}
...
void aim::fire(CUserCmd* cmd)
{
	...
	if (g_ctx.globals.double_tap_aim)
		hitchance_amount = 0;
	else if (!g_ctx.globals.double_tap_aim)
		hitchance_amount = g_cfg.ragebot.weapon[g_ctx.globals.current_weapon].hitchance_amount;

	if (!g_cfg.ragebot.autoscope && g_ctx.globals.weapon->m_zoomLevel() == 0 && g_ctx.globals.weapon->m_iItemDefinitionIndex() == WEAPON_G3SG1 || g_ctx.globals.weapon->m_iItemDefinitionIndex() == WEAPON_SCAR20)
	{
		hitchance_amount = g_cfg.ragebot.weapon[g_ctx.globals.current_weapon].hitchance_amount - 25;
	}
	else
	{
		hitchance_amount = g_cfg.ragebot.weapon[g_ctx.globals.current_weapon].hitchance_amount;
	...
	log << crypt_str("Aimed at ") + (std::string)player_info.szName + crypt_str(": ");
	...
	if (g_cfg.onshot.chams_on_shot_enable)
		chams_on_hit.add_matrix(final_target.record->player, final_target.record->matrixes_data.main);
}
