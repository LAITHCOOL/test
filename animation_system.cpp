...
void lagcompensation::update_player_animations(player_t* e)
{
	...
	if (g_ctx.local()->is_alive() && e->m_iTeamNum() != g_ctx.local()->m_iTeamNum() && !g_cfg.legitbot.enabled)
	{
		...
		memcpy(record->resolver_layers[0], e->get_animlayers(), e->animlayer_count() * sizeof(AnimationLayer));
		memcpy(animstate, &state, sizeof(c_baseplayeranimationstate));

		// first
		animstate->m_flGoalFeetYaw = math::normalize_yaw(e->m_angEyeAngles().y + 66.f);
		...
		memcpy(record->resolver_layers[1], e->get_animlayers(), e->animlayer_count() * sizeof(AnimationLayer));
		memcpy(animstate, &state, sizeof(c_baseplayeranimationstate));
		...
		memcpy(record->resolver_layers[2], e->get_animlayers(), e->animlayer_count() * sizeof(AnimationLayer));
		memcpy(animstate, &state, sizeof(c_baseplayeranimationstate));
